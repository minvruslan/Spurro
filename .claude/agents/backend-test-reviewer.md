...
6. Claims verification: if the writer's report claims green runs or coverage numbers, spot-check by running `pnpm test:integration` and `pnpm test:coverage` from `backend/` (pnpm only, never npm). Run `docker compose stop postgres-test` from the repo root when finished.
