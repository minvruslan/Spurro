...
2. Assertion strength: no test weakened relative to its name — the `it` name promises exactly what the body asserts. Flag tautologies (asserting through the same mechanism under test), assertions on partial fields where the contract parse is required, and `toBeDefined`-style checks where a value is known. Also flag: expectations derived from the same constant or fake as the implementation (ask what still pins the value and its unit — `mins` → `secs` must turn something red); literals pinning a boundary from one side only; `Object.keys` taken after a strip-mode `schema.parse` (extras are already gone); the same window/constant derived in one test file but hardcoded in sibling files.
...
6. Harness discipline: mocks only under `describe("technical")` and narrowest-module; component `call()` for behavior, `app.request` only for HTTP semantics; helpers used instead of inline inserts; catalog tables reset and seeded via `bootstrap*`. Flag any test asserting a trivial oRPC code — the default BAD_REQUEST from input-schema validation or the auth middleware's UNAUTHORIZED — as these test framework behavior, not the module; only statuses declared in the contract `.errors()` may be asserted.
7. Assertion-hole sweep — check every mutating route against these five hole classes; each violation is a finding: (a) every input field's persisted destination asserted from the database, or an explicit ignored-field case; (b) every encrypted-column write verified via raw SQL to contain no plaintext secret; (c) every cascade's end state asserted per affected entity type, not just row existence; (d) sibling survival asserted for every scoped mutation (dropping a `where` scope must turn a test red); (e) every contract error with an explicit HTTP status asserted on the wire; (f) every rejection path asserted side-effect-free — no rows written or changed and no external calls (`not.toHaveBeenCalled()`), so that reordering an external call ahead of a guard turns a test red; (g) every retry/cleanup path asserted to actually reach the external system, not just flip database state.
...
Every finding is worded problem-first in plain everyday language: first what concretely is broken and what it leads to ("тест всегда проходит, даже если код сломают, потому что ..."), then the fix. No coined jargon or compressed reviewer shorthand ("вакуумный ассерт", "пиннит границу") — category labels from the severity list are fine, but the body of each finding must be understandable on first pass. Names of fields, statuses, and constants from the codebase stay as-is.
