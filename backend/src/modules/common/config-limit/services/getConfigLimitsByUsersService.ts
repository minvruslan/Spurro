...
import { findConfigLimitsByUsers } from "../queries/findConfigLimitsByUsers.js"
import { countConfigsByProtocolTypeForUsers } from "../queries/countConfigsByProtocolTypeForUsers.js"
...
export async function getConfigLimitsByUsersService(
  ...
  const [limitRows, usageRows] = await Promise.all([
    findConfigLimitsByUsers(executor, userIds),
    countConfigsByProtocolTypeForUsers(executor, userIds),
  ])
